...
data = pd.read_csv("/Users/yuhangzhang/Desktop/criteo_sample_50w.csv")
...
model.to(device)
loss_fcn = nn.BCELoss()  # Loss函数:二分类交叉熵损失
...
def train_and_eval(model, train_loader, valid_loader, epochs, device):
    ...
    for _ in range(epochs):
        ...
        scheduler.step()
        """推断部分"""
        # 关闭batch normalization和drop out
        model.eval()
        with torch.no_grad():
...
if __name__ == '__main__':
    train_and_eval(model, train_loader, valid_loader, 15, device)